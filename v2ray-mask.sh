#!/bin/bash
# v2ray centos系统一键安装脚本 基于 hijk<https://hijk.art> 改造
# Author: Jaluik 
...
getData() {
    ...
    echo "   2) 小说站(随机选择)"
    echo "   3) 美女站(https://anime-pictures.net)"
    echo "   4) 高清壁纸站(https://anime-pictures.net)"
    echo "   5) 自定义反代站点(需以http或者https开头)"
    read -p "  请选择伪装网站类型[默认:高清壁纸站]" answer
    if [[ -z "$answer" ]]; then
        PROXY_URL="https://anime-pictures.net"
    else
        case $answer in
        1)
            PROXY_URL=""
...
installV2ray() {
    ...
    bash <(curl -sL ${V6_PROXY}https://raw.githubusercontent.com/jaluik/v2ray-mask/master/goV2.sh)
